...
def main() -> int:
    ...
    client.request_wsh_event_data(req_id=req_id, contract=contract, filter_payload=filter_payload)

    deadline = time.time() + args.timeout_seconds
    while time.time() < deadline and memory_repo.first_event is None and app.entitlement_error is None:
        time.sleep(0.25)
    ...
    if memory_repo.first_event is None:
        print("No WSH event payload received before timeout.")
        if app.last_error:
            req, code, message = app.last_error
            print(f"Last IB error: reqId={req}, code={code}, message={message}")
        return 4

    event = memory_repo.first_event
    print(f"Stored corporate event req_id={event.req_id} symbol={event.symbol}")
    print(f"Parsed top-level keys: {list(event.json_payload.keys())}")
